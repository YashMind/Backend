...
async def register_whatsapp_user(request: Request, db: Session = Depends(get_db)):

    data = await request.json()
    if "bot_id" not in data or "whatsapp_number" not in data:
        raise HTTPException(
            status_code=400,
            detail="Incomplete request data: bot_id and phone number are required",
        )
    ...
    if not whatsapp_number.startswith("+"):
        raise HTTPException(
            status_code=400,
            detail="WhatsApp number must include country code (e.g., +1234567890)",
        )
    ...
    if existing:
        raise HTTPException(
            status_code=400, detail="This WhatsApp number is already registered"
        )

    # Save to DB
    new_user = WhatsAppUser(
        bot_id=bot_id, whatsapp_number=whatsapp_number, created_at=datetime.utcnow()
    )
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Failed to send welcome message: {str(e)}"
        )
...
async def handle_whatsapp_message(
    ...
    try:
        ...
        user = (
            db.query(WhatsAppUser).filter_by(whatsapp_number=From.split(":")[1]).first()
        )
        if not user:
            print("User not found")
            return Response(
                status_code=404
            )  # Ignore messages from unregistered numbers
        ...
        # Send reply
        twilio_client.messages.create(
            from_=f"whatsapp:{twilio_number}", body=response_text, to=From
        )
