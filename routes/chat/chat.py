...
# get all charts
@router.get("/chats/", response_model=ChatSessionWithMessages)
async def list_chats(request: Request, db: Session = Depends(get_db)):
    try:
        ...
        return [{"chat_id": c.id, "title": c.title, "created_at": c.created_at} for c in chats]
    except HTTPException as http_exc:
        raise http_exc
    except Exception as e:
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def get_chat_history(chat_id: int, request: Request, db: Session = Depends(get_db)):
    try:
        ...
        messages = db.query(ChatMessage).filter_by(chat_id=chat_id).order_by(ChatMessage.created_at.asc()).all()
        return [{"sender": m.sender, "message": m.message, "time": m.created_at} for m in messages]
    except HTTPException as http_exc:
        raise http_exc
    except Exception as e:
        raise HTTPException(status_code=500, detail="Internal server error")
