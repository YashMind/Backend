...
from schemas.supportTickets.schema import EmailRequest, TicketCreate, TicketResponse, TicketStatusUpdate, TicketAssign
...
@public_route()
def create_ticket(request:Request,ticket: TicketCreate, db: Session = Depends(get_db)):
...
def get_all_tickets(request:Request,skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
@check_permissions(['support-communication'])
def update_status(request:Request,ticket_id: int, status: TicketStatusUpdate, db: Session = Depends(get_db)):
    ticket = db.query(SupportTicket).filter(SupportTicket.id == ticket_id).first()
    if not ticket:
        raise HTTPException(status_code=404, detail="Ticket not found")
...
@check_permissions(['support-communication'])
def assign_ticket(request:Request,ticket_id: int, assign: TicketAssign, db: Session = Depends(get_db)):
    ticket = db.query(SupportTicket).filter(SupportTicket.id == ticket_id).first()
    if not ticket:
        raise HTTPException(status_code=404, detail="Ticket not found")
...
def send_status_notification(
    ...
    if not ticket.user.email:
        raise HTTPException(status_code=400, detail="Ticket has no associated user email")
